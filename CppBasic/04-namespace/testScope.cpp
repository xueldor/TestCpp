...
void autoVariable(){
	...
	using namespace std;
    //C++11已经删除了这一用法，auto用于变量的自动类型推断。故注释掉
...
extern int global4 = 46;//这是定义，所有文件中不能重复出现定义。编译器给警告。不要这么写，如果是定义，不要额extern，如果是声明，不要赋值。
...
int main_scope(){
    ...
    extern int ccc;//ccc定义在本文件后面，也要用extern声明
    std::cout<<ccc;
